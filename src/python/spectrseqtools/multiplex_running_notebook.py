...
fastafile = SeqIO.parse("C:\\Users\\juan\\Documents\\Postdoc\\modified_tRNA_Gly_all_rna_sequences.fasta", "fasta")
...
NUCLEOTIDE_DF = NucleotideAlphabet.from_file(ErrorCalculator.with_metric()).to_dataframe()
...
def reference_sequence(sequence_string):
    ...
    for n in list(sequence_string):
        ...
        id_list = NUCLEOTIDE_DF.filter(pl.col("names").list.contains(encoding_to_id)).select("names").item().to_list()
...
options = Options(
        ...
        prediction = PredictionOptions(
                    ...
                    fragment_predictions = Path(f"output/{sample_name}/prediction/fragments.prediction.tsv"), 
                    sequence_prediction = Path(f"output/{sample_name}/prediction/fragments.prediction.fasta"), 
...
(
    ...
    singletons, 
    meta
    ) = multiplexing.pre_process_multiplexing(
        options = options, 
...
singletons.write_csv(str(options.preprocessing.output_dir)+"preprocessed.singletons.tsv", separator="\t")
fragments.write_csv(str(options.preprocessing.output_dir)+"preprocessed.fragments.tsv", separator="\t")
with open(
    str(options.preprocessing.output_dir)+"preprocessed.meta.yaml", "w", encoding="utf-8"
) as f:
    yaml.dump(meta, f)
...
fragments = pl.read_csv(str(options.preprocessing.output_dir)+"preprocessed.fragments.tsv", separator = "\t")
singletons = pl.read_csv(str(options.preprocessing.output_dir)+"preprocessed.singletons.tsv", separator = "\t")
...
for grp_number in tqdm.tqdm(grp_numbers, desc= "Performing prediction"):
    ...
    try:
        raw_fragments, prediction_fragments, fasta_dict = Predictor(options.prediction).predict()
        prediction_fragments = prediction_fragments.with_columns(pl.lit(grp_number).alias("ms1_mass_group"))
...
with open(str(options.prediction.sequence_prediction), "w") as f:
    for header, sequence in all_fasta_dicts.items():
        f.write(f"{header}\n")
        f.write(f"{sequence}\n")
raw_fragments.write_csv(str(options.prediction.output_dir)+"preprocessed.fragments.standard_unit_fragments.tsv", separator = "\t")
prediction_fragments.write_csv(str(options.prediction.fragment_predictions), separator = "\t")
...
sequence_dict = {}
with open(
    str(options.prediction.sequence_prediction), 
    mode="r", 
    encoding="utf-8"
    ) as f:
...
def align_prediction_results(prediction_vals, target_sequence, alignment_score_threshold = 0):
    ...
    for i in tqdm.tqdm(prediction_vals, desc = "Calculating alignment scores"):
        ...
        alignment_vals.append(final_comparison+(i["group_number"], 
                                                i["intact_mass"], 
                                                i["min_window_time"], 
                                                i["max_window_time"], 
                                                i["adduct_types"]))

    df_alignment = pl.DataFrame(alignment_vals, schema = ["predicted_string", 
...
def interpret_alignment_results(df_alignment, target_sequence):
    ...
    df_expanded_alignment = df_expanded_alignment.sort(["score", 
                                                        "group", ""
                                                        "target_position"])
